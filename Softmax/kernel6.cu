...
__global__ void __kernel_fused_softmax_block_level(float *A, int m, int n){
    ...
    if(row<m){
        float *Current_Row = A+row*n;
        __shared__ float data[WARP_SIZE];
        ...
        //reduce in each warp with warp_level instructions
        float global_max_val = max_val;

        for(int offset=16; offset>0; offset>>=1){
            global_max_val = max(global_max_val, __shfl_down_sync(0xffffffff, global_max_val, offset));
        }
        //each line_id=0 gains the max val in its warp
        if(line_id==0){
            data[warp_id] = global_max_val;
        }
        __syncthreads();

        //use the first warp to reduce again

        if(warp_id==0){
            global_max_val = data[line_id];
            int mask = (1<<warp_num)-1; //only first warp_num threads gain the valid value
            for(int offset=warp_num/2; offset>0; offset>>=1){
                global_max_val = max(global_max_val, __shfl_down_sync(mask, global_max_val, offset));
            }

            if(line_id==0){
                data[0] = global_max_val;
            }
        }

        __syncthreads();
        global_max_val = data[0];
        ...
        //reduce the norm sum
        for(int offset=16; offset>0; offset>>=1){
            norm_base += __shfl_down_sync(0xffffffff, norm_base, offset);
        }
        //each line_id=0 gains the max val in its warp
        if(line_id==0){
            data[warp_id] = norm_base;
        }
        __syncthreads();
        ...
        if(warp_id==0){
            norm_base = data[line_id];
            int mask = (1<<warp_num)-1; //only first warp_num threads gain the valid value
            for(int offset=warp_num/2; offset>0; offset>>=1){
                norm_base += __shfl_down_sync(mask, norm_base, offset);
            }
